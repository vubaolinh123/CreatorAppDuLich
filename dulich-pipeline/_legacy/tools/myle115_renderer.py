...
def render_cover(bg_path: str, hook: str, out_path: str,
    ...
    max_line_w = W - bg_pad_x * 2 - 24
    font_size = 74
    font_title = beviet_bold(font_size)
    for line in lines:
        while int(draw.textlength(line, font=font_title)) > max_line_w and font_size > 44:
            font_size -= 2
            font_title = beviet_bold(font_size)
    ...
    # Subtitle — noto 26px, centered
    font_sub = noto(26)
    sub = "Đà Lạt nhiều điểm check - in"
    sw = int(draw.textlength(sub, font=font_sub))
    draw.text(((W - sw) // 2, y), sub, font=font_sub, fill=(220, 220, 220))
    y += font_sub.size + 14
...
def render_grid_slide(venues_4: list, category_label: str, out_path: str) -> str:
    ...
    # Top-row names: top-left corner of each top cell
    for vn, px_off in zip(venues_4[:2], [0, CW]):
        _name_tag(draw, px_off + pad, pad, vn.get("name", ""), font_name)

    # Bottom-row names: bottom-left corner of each bottom cell
    name_y_bot = H - font_name.size - 8 - pad
    for vn, px_off in zip(venues_4[2:], [0, CW]):
        _name_tag(draw, px_off + pad, name_y_bot, vn.get("name", ""), font_name)

    # Category label: Anton white + #8c0808 thin outline
